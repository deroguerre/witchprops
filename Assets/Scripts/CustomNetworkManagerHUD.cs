        ...
        void OnGUI()
        {
            ...
            if (!manager.IsClientConnected() && !NetworkServer.active && manager.matchMaker == null)
            {
                if (noConnection)
                {
                    if (UnityEngine.Application.platform != RuntimePlatform.WebGLPlayer)
                    {
                        if (GUI.Button(new Rect(xpos, ypos, 200, 20), "Héberger une partie (H)"))
                        {
                            manager.StartHost();
                        }
                        ypos += spacing;
                    ...
                    if (GUI.Button(new Rect(xpos, ypos, 200, 20), "Rejoindre une partie (C)"))
                    {
                        manager.StartClient();
                    }

                    manager.networkAddress = GUI.TextField(new Rect(xpos + 210, ypos, 95, 20), manager.networkAddress);
                    ypos += spacing;
            ...
            {
                if (GUI.Button(new Rect(xpos, ypos, 200, 20), "Déconnecter (X)"))
                {
                    manager.StopHost();
                }
                ypos += spacing;
